...
export function DebtsPanel() {
  ...
  const [editKind, setEditKind] = useState<DebtKind>("debt");
  ...
  const [paymentRowError, setPaymentRowError] = useState<{
    id: string;
    message: string;
  } | null>(null);
  ...
  async function handleSaveEdit(id: string) {
    setRowError(null);
    try {
      await debtsRepository.update(id, {
        kind: editKind,
  ...
  function handleStartEditPayment(payment: {
    id: string;
    accountId: string;
    amount: number;
    date: string;
  ...
  async function handleSaveEditPayment(id: string) {
    setPaymentRowError(null);
    try {
      await debtPaymentsRepository.update(id, {
  ...
  const debtReferenceById = new Map((debtsForPaymentForm ?? []).map((d) => [d.id, d.reference]));
              ...
              {summaries.map(({ debt, remaining, status, plannedMonthlyPayment }) =>
                editingId === debt.id ? (
                  <tr key={debt.id}>
                    ...
                    <td>
                      <select
                        aria-label={`Type de ${debt.reference}`}
                        value={editKind}
                        onChange={(e) => setEditKind(e.target.value as DebtKind)}
                      >
                        <option value="debt">Dette</option>
                        <option value="receivable">Créance</option>
                      </select>
                    ...
                    <td>
                      <input
                        aria-label={`Tiers de ${debt.reference}`}
                    ...
                    <td>
                      <select
                        aria-label={`Compte de ${debt.reference}`}
                        value={editAccountId}
                    ...
                    <td>
                      <input
                        aria-label={`Montant de ${debt.reference}`}
                        type="number"
                        value={editAmount}
                    ...
                    <td>
                      <input
                        aria-label={`Échéance de ${debt.reference}`}
                        type="date"
                        value={editDueDate}
                    ...
                    <td>
                      <button type="button" onClick={() => handleSaveEdit(debt.id)}>
                        Enregistrer
                      </button>{" "}
