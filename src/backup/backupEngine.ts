...
function assertValidBackup(value: unknown): asserts value is BackupFile {
  if (typeof value !== "object" || value === null) {
    throw new InvalidBackupError("Ce fichier n'est pas une sauvegarde SEZZ valide.");
  }
  const candidate = value as Record<string, unknown>;
  if (candidate["version"] !== BACKUP_FORMAT_VERSION) {
    throw new InvalidBackupError(
      "Ce fichier provient d'une version incompatible de SEZZ, ou n'est pas une sauvegarde.",
    );
  }
