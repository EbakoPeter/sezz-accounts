...
- **Synchronisation entre appareils.** Un serveur séparé (`sezz-accounts-server/`)
...
- **Rejoindre un foyer existant via synchronisation, sans créer de nouvel
  administrateur.** Un appareil réellement neuf peut maintenant se connecter
  directement au compte de synchronisation déjà utilisé ailleurs, dès
  l'écran « premier lancement » — avant toute création locale, évitant de
  générer une seconde clé de chiffrement indépendante (voir SECURITY.md).
- **Modification des enregistrements.** Comptes, opérations, budget et
  dettes se modifient tous depuis l'interface, pas seulement à la création —
  y compris changer le compte ou le type d'une opération, ou le type
  dette/créance d'une dette, qui n'étaient pas prévus au départ.
- **Solde cumulé de tous les comptes**, affiché en bas du tableau des
  comptes.
- **Transferts entre comptes.** Un transfert déplace correctement l'argent
  d'un compte à l'autre sans jamais apparaître comme revenu ou dépense dans
  le Rapport Mensuel ou les Recommandations.
- **Engagements sur le budget.** De l'argent réservé pour une dépense prévue
  mais pas encore payée, avec son propre statut (engagé/réalisé/annulé) et
  sa propre colonne dans le tableau prévisionnel/réel — un troisième état
  entre « alloué » et « dépensé ».
- **Blocage des dépenses hors budget.** Une dépense imputée à une ligne
  budgétaire est refusée (même alerte rouge que partout ailleurs) si elle
  dépasse ce qui reste réellement disponible — alloué moins déjà dépensé
  moins déjà engagé. Les lignes non provisionnées restent sans plafond,
  comme ailleurs dans l'app.
- **Trois rapports téléchargeables en PDF**, onglet Rapports : un rapport
  général (comptes, revenus/dépenses, budget du mois), un rapport par
  opérations (relevé d'une période), et un rapport de trésorerie
  (évolution du solde de chaque compte, mois par mois).
- 471 tests automatisés côté client (dépôts + composants) + 22 côté serveur,
  intégralement verts.
...
src/
  ...
  db/
    ...
    deletionLog.ts                       Journal des suppressions locales, pour que le moteur
                                          de synchronisation sache quoi transformer en tombstone
