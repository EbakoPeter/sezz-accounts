...
## Limites connues, résolues depuis

- **Un nouvel appareil peut désormais « rejoindre » un foyer existant via la
  synchronisation, sans jamais créer sa propre clé de chiffrement.** C'était
  une vraie limite jusqu'ici : l'écran « premier lancement » ne menait qu'à
  « créez le compte administrateur principal », qui génère toujours sa
  propre DEK — la seule façon d'éviter le décalage de clé décrit ci-dessous
  était une manipulation manuelle risquée. L'écran de premier lancement
  propose maintenant un second choix, « Rejoindre un foyer existant via
  synchronisation » : il connecte l'appareil au compte de synchronisation
  déjà utilisé ailleurs et récupère immédiatement les utilisateurs et
  données existants, _avant_ toute création locale. Se connecter ensuite
  avec un identifiant/mot de passe déjà utilisé sur l'autre appareil
  déverrouille la même DEK partagée — le déverrouillage ne dépend que du mot
  de passe, jamais de l'appareil qui le tape.
...
- **Un enregistrement ignoré à cause d'un décalage de clé ne l'est encore
  que silencieusement.** `fromStorageRows` (ci-dessus) laisse une trace dans
  la console du navigateur, mais rien de visible dans l'application
  elle-même — un indicateur sur l'onglet Synchronisation reste à construire.
